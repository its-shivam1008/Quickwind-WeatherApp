...
export default class Forecast extends Component {
    constructor(){
      ...
      async customInitialization(){
        ...
        let forc = "https://api.weatherapi.com/v1/forecast.json?key=11c20f2803f04f099ac160306242101&q="+place+"&days=7";
      ...
      <>
        <div className='hidden lg:grid bg-white h-fit py-10 bg-opacity-15 backdrop-blur-xl mt-14 mx-7 rounded-[20px] shadow-[#009e9a] shadow-2xl'>
              <div className='flex text-5xl text-cyan-700 cursor-pointer hover:text-white transition-colors duration-500 p-5 my-2'>
                Next 7 Days in <p className='ml-5 text-white'>{this.state.location.name}</p>
              </div>
              <div className='grid grid-cols-7 gap-4 text-white mx-10'>
                {this.state.forecast.forecastday.map((element,str)=>{
                    ...
                    <div className='flex justify-center text-2xl font-bold text-[#edcc37] hover:text-white transition-colors duration-500 text-center'>
